...
export type DbUser = {
  ...
  /** Opt-out (default true): digest email for unanswered product Q&A on listings. */
  ...
  /** Opt-out (default true): email + in-app when followed shops list new products. */
...
function mapRow(row: AccountRow): DbUser {
  return {
    ...
    notifyShopFollows: row.notify_shop_follows !== false,
    image: row.image,
...
const SELECT_COLS = `
  ...
  COALESCE(notify_shop_follows, TRUE) AS notify_shop_follows,
...
export async function updateQaDigestPreferences(
  id: string,
  prefs: { notifyQaDigest: boolean }
): Promise<DbUser | null> {
  const existing = await findUserById(id)
  if (!existing) return null
  await query(
    `UPDATE accounts
     SET notify_qa_digest = $2,
         updated_at = NOW()
     WHERE id = $1`,
    [id, Boolean(prefs.notifyQaDigest)]
  )
  return findUserById(id)
}

export async function updateBackInStockPreferences(
  id: string,
  prefs: { notifyBackInStock: boolean }
): Promise<DbUser | null> {
  const existing = await findUserById(id)
  if (!existing) return null
  await query(
    `UPDATE accounts
     SET notify_back_in_stock = $2,
         updated_at = NOW()
     WHERE id = $1`,
...
export async function updateShopFollowPreferences(
  id: string,
  prefs: { notifyShopFollows: boolean }
): Promise<DbUser | null> {
  const existing = await findUserById(id)
  if (!existing) return null
  await query(
    `UPDATE accounts
     SET notify_shop_follows = $2,
         updated_at = NOW()
     WHERE id = $1`,
    [id, Boolean(prefs.notifyShopFollows)]
  )
  return findUserById(id)
}
